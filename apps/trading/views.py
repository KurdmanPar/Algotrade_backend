...
from .serializers import OrderSerializer, TradeSerializer, PositionSerializer, OrderLogSerializer
from apps.core.views import SecureModelViewSet


class OrderViewSet(SecureModelViewSet):
    queryset = Order.objects.all()  # اضافه شد
...
class PositionViewSet(SecureModelViewSet):
    queryset = Position.objects.all()  # اضافه شد
